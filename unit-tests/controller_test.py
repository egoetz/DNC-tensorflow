...
class DummyController(BaseController):
    def network_vars(self):
        self.W = tf.Variable(tf.random.truncated_normal([self.nn_input_size, 64]))
    ...
    def test_process_input(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                self.assertEqual(
                    np.concatenate([np.reshape(val, (2, -1)) for _, val in zeta.items()], axis=1).shape, (2, 38))

                self.assertEqual(rv.shape, (2, 10))
                self.assertEqual(
                    np.concatenate([np.reshape(val, (2, -1)) for _, val in rzeta.items()], axis=1).shape, (2, 38))
