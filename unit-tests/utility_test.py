...
class DNCUtilityTests(unittest.TestCase):
    ...
    def test_unstack_into_tensorarray(self):
        graph = tf.Graph()
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                T = tf.random.normal([5, 10, 7, 7])
                ta = util.unstack_into_tensorarray(T, axis=1)
    ...
    def test_stack_into_tensor(self):
        graph = tf.Graph()
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                T = tf.random.normal([5, 10, 7, 7])
                ta = util.unstack_into_tensorarray(T, axis=1)
                pT = util.stack_into_tensor(ta, axis=1)
