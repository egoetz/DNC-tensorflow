    ...
    def test_lookup_weighting(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                predicted_weights = np.exp(sim) / np.sum(np.exp(sim), axis=1, keepdims=True)

                memory_matrix = tf.convert_to_tensor(value=initial_mem)
                op = mem.get_lookup_weighting(memory_matrix, keys, strengths)
                c = session.run(op)

                self.assertEqual(c.shape, (2, 4, 2))
                self.assertTrue(np.allclose(c, predicted_weights))
    ...
    def test_update_usage_vector(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                read_weightings = tf.convert_to_tensor(value=init_read_weightings)
                write_weighting = tf.convert_to_tensor(value=init_write_weightings)
                usage_vector = tf.convert_to_tensor(value=init_usage)
            ...
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                for i in range(2):
                    for j in range(4):
                        ...
                        predicted_weights[i, free_list[i, j]] = (1 - mock_usage[i, free_list[i, j]]) * \
    ...
    def test_updated_write_weighting(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                predicted_weights = write_gate * (allocation_gate * allocation_weighting + (1 - allocation_gate) *
                                                  np.squeeze(lookup_weighting))
    ...
    def test_update_memory(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                memory_matrix = tf.convert_to_tensor(value=memory_matrix)
    ...
    def test_update_precedence_vector(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                precedence_vector = tf.convert_to_tensor(value=initial_precedence)
    ...
    def test_update_link_matrix(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                precedence_vector = tf.convert_to_tensor(value=_precedence_vector)
    ...
    def test_get_directional_weightings(self):
        ...
        with graph.as_default():
            with tf.compat.v1.Session(graph=graph) as session:
                ...
                read_weightings = tf.convert_to_tensor(value=_read_weightings)
