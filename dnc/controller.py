...
class BaseController:
    ...
    def __init__(self, input_size, output_size, memory_read_heads, memory_word_size, batch_size=1):
        ...
        with tf.compat.v1.name_scope("controller"):
            ...
            with tf.compat.v1.variable_scope("shape_inference"):
    ...
    def initials(self):
        ...
        self.interface_weights = tf.Variable(
            tf.random.normal([self.nn_output_size, self.interface_vector_size], stddev=0.1),
            name='interface_weights'
        )
        self.nn_output_weights = tf.Variable(
            tf.random.normal([self.nn_output_size, self.output_size], stddev=0.1),
            name='nn_output_weights'
        )
        self.mem_output_weights = tf.Variable(
            tf.random.normal([self.word_size * self.read_heads, self.output_size], stddev=0.1),
