...
class FeedforwardController(BaseController):
    ...
    def network_vars(self):
        initial_std = lambda in_nodes: np.min(np.sqrt(2.0 / in_nodes), initial=1e-2)
        input_ = self.nn_input_size

        self.W1 = tf.Variable(tf.random.truncated_normal([input_, 128], stddev=initial_std(input_)), name='layer1_W')
        self.W2 = tf.Variable(tf.random.truncated_normal([128, 256], stddev=initial_std(128)), name='layer2_W')
    ...
    def initials(self):
        ...
        self.interface_weights = tf.Variable(
            tf.random.truncated_normal([self.nn_output_size, self.interface_vector_size],
                                stddev=initial_std(self.nn_output_size)),
            name='interface_weights'
        )
        self.nn_output_weights = tf.Variable(
            tf.random.truncated_normal([self.nn_output_size, self.output_size], stddev=initial_std(self.nn_output_size)),
            name='nn_output_weights'
        )
        self.mem_output_weights = tf.Variable(
            tf.random.truncated_normal([self.word_size * self.read_heads, self.output_size],
